//
//
//  Generated by StarUML(tm) C++ Add-In
//
//  @ Project : Phoenix
//  @ File Name : Packet.cpp
//  @ Date : 2/9/2010
//  @ Author :
//
//

#include "core/Packet.h"
#include "util/crc.h"

//#include "boards/backplane/dbg_led.h"

using namespace std;

namespace Phoenix
{
    namespace Core
    {
        std::size_t Packet::packetCounter = 0;

        Packet::Packet(void )
        {
            messagePtr = NULL;
            source = LOCATION_ID_INVALID;
            number = 0;
            destination = LOCATION_ID_INVALID;
            timestamp = 0;
        }

        Packet::Packet(LocationIDType sourceIn, LocationIDType destIn, uint16 numberIn, uint32 timestampIn, Message * messageIn)
        {
            source = sourceIn;
            destination = destIn;
            timestamp = timestampIn;
            if (messageIn != NULL)
            {
                messagePtr = messageIn->Duplicate();
            }
            else
            {
                messagePtr = NULL;
                number = numberIn;
                return;
            }
			
            if (NULL == messagePtr)
            {
                source = LOCATION_ID_INVALID;
                destination = LOCATION_ID_INVALID;
                number = 0;
                return;
            }
			
            if(messagePtr->IsResponse())
            {
                number = numberIn;
            }
            else
            {
                number = packetCounter++;
            }
        }

        Packet::Packet(uint8 * buffer, std::size_t size)
        {
            // Check that the buffer is sized correctly.
            if (NULL == buffer || size < (sizeof(LocationIDType) + sizeof(LocationIDType) + sizeof(uint16) + sizeof(uint32)))
            {
                source = LOCATION_ID_INVALID;
                destination = LOCATION_ID_INVALID;
                messagePtr = NULL;
                return;
            }

            // Get the source
            source = (LocationIDType)(((uint16)(buffer[0]) << 8) | buffer[1]);
            buffer += sizeof(uint16);
            size -= sizeof(uint16);
            destination = (LocationIDType)(((uint16)(buffer[0]) << 8) | buffer[1]);
            buffer += sizeof(uint16);
            size -= sizeof(uint16);
            number = (uint16)(((uint16)(buffer[0]) << 8) | buffer[1]);
            buffer += sizeof(uint16);
            size -= sizeof(uint16);
            timestamp = (uint32)(((uint32)(buffer[0]) << 24) | ((uint32)(buffer[1]) << 16) | ((uint32)(buffer[0]) << 8) | ((uint32)(buffer[3])));
            buffer += sizeof(uint32);
            size -= sizeof(uint32);

            messagePtr = Message::CreateMessage(buffer, size);

            // Check that the allocation worked correctly.
            if (NULL == messagePtr)
            {
                source = LOCATION_ID_INVALID;
                destination = LOCATION_ID_INVALID;
                return;
            }

			size -= messagePtr->GetFlattenSize();;
			
			if (size < 2)
			{
				source = LOCATION_ID_INVALID;
				destination = LOCATION_ID_INVALID;
				delete messagePtr;
				messagePtr = NULL;
				return;
			}
        }

        Packet::Packet(const Packet & packetSource)
        {
            if (NULL == packetSource.GetMessagePtr())
            {
                source = LOCATION_ID_INVALID;
                destination = LOCATION_ID_INVALID;
                number = 0;
                timestamp = 0;
                messagePtr = NULL;
                return;
            }
            messagePtr = packetSource.GetMessagePtr()->Duplicate();

            if (NULL == messagePtr)
            {
                source = LOCATION_ID_INVALID;
                destination = LOCATION_ID_INVALID;
                number = 0;
                timestamp = 0;
                return;
            }
			
            destination = packetSource.destination;
            source = packetSource.source;
            number = packetSource.number;
            timestamp = packetSource.timestamp;
        }

        Packet::~Packet(void )
        {
            delete messagePtr;
            messagePtr = NULL;
        }

        Packet & Packet::operator=(const Packet & source)
        {
            if (this == &source)
            {
                return *this;
            }

            SetSource(source.GetSource());
            SetDestination(source.GetDestination());
            SetNumber(source.GetNumber());
            SetTimestamp(source.GetTimestamp());
            if (NULL == source.GetMessagePtr())
            {
                messagePtr = NULL;
            }
            else
            {
                messagePtr = source.GetMessagePtr();
            }

            return *this;
        }


        LocationIDType Packet::GetDestination(void) const
        {
            return destination;
        }

        LocationIDType Packet::GetSource(void) const
        {
            return source;
        }

        uint16 Packet::GetNumber(void) const
        {
            return number;
        }

        uint32 Packet::GetTimestamp(void) const
        {
            return timestamp;
        }

        Message * Packet::GetMessagePtr(void) const
        {
            return messagePtr;
        }

        std::size_t Packet::GetPacketCounter(void) const
        {
            return packetCounter;
        }

        std::size_t Packet::GetFlattenSize(void ) const
        {
            std::size_t messageSize;

            if (NULL == messagePtr)
            {
                messageSize = 0;
            }
            else
            {
                messageSize = messagePtr->GetFlattenSize();
            }

            return sizeof(source) + sizeof(destination) + sizeof(number) + sizeof(timestamp) + messageSize + sizeof(crc_t);
        }

        bool Packet::operator==(const Packet & check) const
		{
            if (messagePtr == NULL)
            {
                return false;
            }

            return ((source == check.source) &&
                    (destination == check.destination) &&
                    (number == check.number) &&
                    (timestamp == check.timestamp) &&
                    (*messagePtr == check.messagePtr));
		}

        void Packet::SetSource(LocationIDType newSource)
        {
            source = newSource;
        }

        void Packet::SetDestination(LocationIDType newDestination)
        {
            destination = newDestination;
        }

        void Packet::SetNumber(uint16 newNumber)
        {
            number = newNumber;
        }

        void Packet::SetTimestamp(uint32 newTimestamp)
        {
            timestamp = newTimestamp;
        }

        void Packet::SetMessage(Message * newMessage)
        {
            delete messagePtr;
			if(newMessage == NULL)
			{
				messagePtr = NULL;
				return;
			}
            messagePtr = newMessage->Duplicate();
        }

        std::size_t Packet::Flatten(uint8 * buffer, std::size_t size) const
        {
            size_t numCopied = 0, messageCopied = 0;
            uint8 * bufferStart = buffer;
            crc_t crc;

            // Check the buffer and buffer size.
            if (NULL == buffer || size < GetFlattenSize( ))
            {
                return 0;
            }

            // Copy the source, destination, number, messagePtr
            *(buffer++) = (source >> 8) & 0xFF;
            *(buffer++) = source & 0xFF;
            numCopied+=2;
            size-=2;

            *(buffer++) = (destination >> 8) & 0xFF;
            *(buffer++) = destination & 0xFF;
            numCopied+=2;
            size-=2;

            *(buffer++) = (number >> 8) & 0xFF;
            *(buffer++) = number & 0xFF;
            numCopied+=2;
            size-=2;

            *(buffer++) = (timestamp >> 24) & 0xFF;
            *(buffer++) = (timestamp >> 16) & 0xFF;
            *(buffer++) = (timestamp >> 8)  & 0xFF;
            *(buffer++) = timestamp & 0xFF;
            numCopied+=4;
            size-=4;

            messageCopied += messagePtr->Flatten(buffer, size);
            numCopied += messageCopied;
            buffer += messageCopied;

           // crc = crcFast(bufferStart, numCopied, INITIAL_REMAINDER);
            for (size_t i = 0; i < sizeof(crc); ++i)
            {
                *(buffer++) = (crc >> (8*(sizeof(crc)-i-1))) & 0xff;
            }
            numCopied += sizeof(crc);

            return numCopied;
		}
    }
}
