...
=======
	//if Subsystem command
		...
		char * subsystem = strtok(*line,",");
		char * cmd = strtok(*line,",");
		uint8 * messagebuff = strtok(*line,",");
		if (strcmp(subsystem,"ACS") == 0){
			...
			if (msg == NULL){
				...
				ACPPacket * opcode = new ACPPacket(SERVER_LOCATION_ACS, HARDWARE_LOCATION_ACS, cmd, buffLength, messagebuff);
				ACPPacket * ret = DispatchPacket(opcode);
			}
			...
			if (msg == NULL){
				...
				uint16 buffLength = sizeof(messagebuff);
				ACPPacket * opcode = new ACPPacket(SERVER_LOCATION_COM, HARDWARE_LOCATION_COM, cmd, buffLength, messagebuff);
				ACPPacket * ret = DispatchPacket(opcode);
			}
			...
			if (msg == NULL){
				...
				uint16 buffLength = sizeof(messagebuff);
				ACPPacket * opcode = new ACPPacket(SERVER_LOCATION_EPS, HARDWARE_LOCATION_EPS, cmd, buffLength, messagebuff);
				ACPPacket * ret = DispatchPacket(opcode);
			}
			...
			if (msg == NULL){
				...
				uint16 buffLength = sizeof(messagebuff);
				ACPPacket * opcode = new ACPPacket(SERVER_LOCATION_PLD, HARDWARE_LOCATION_PLD, cmd, buffLength, messagebuff);
				ACPPacket * ret = DispatchPacket(opcode);
			}
		}else{
