...
/*
* Function: isEqual
* Purpose: Compares values 1 - 3 to see if they are the same. If not
*  function returns 0, else it returns 1.
* Inputs:
*  lastVal     = first array, 3 floats long
*  currentVal  = second array, 3 floats long
* Outputs:
*  returns 1 if all values equal, else it returns 0.
*/
int isEqual(float lastVal[4], float currentVal[4])
{
	...
	for (i = 1; i < 4; i++) {
		if (lastVal[i] != currentVal[i]) {
			return 0;
		}
	}
	//else, if we have made it this far, they must be the same
	return 1;
}
