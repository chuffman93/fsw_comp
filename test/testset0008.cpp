/*! \file testset0008.cpp
 * Created on: Jul 9, 2014
 *    Author: fsw
 *
 *  \brief Test 8 of the Setting Class.
 *
 *	This test verifies that the get flatten method works
 *	correctly for a Setting with ConfigItem.
 *
 */
...
TEST(TestSetting, GetFlattenSizeConfigItem) {
	uint8 * buffer = new uint8[BUFFER_SIZE];

	VariableTypeData v1(TEST_VALUE);

	// Setting to be flattened
	Setting s1in(TEST_CONFIG, v1);
